...
int fault_on_finalization_phase(unsigned char *state, const unsigned char *k, unsigned char diff, unsigned char pos) {
	//int i;
	...
	//fault_on_permutation_256(state, AROUNDS, diff);
	
	fault_on_permutation_256_final_phase(state, AROUNDS, diff, pos);
...
int finalization_phase(unsigned char *state, const unsigned char *k) {
	...
	permutation_256_final_phase(state, AROUNDS);
...
int fault_on_crypto_aead_decrypt(
	unsigned char *m,
	unsigned long long *mlen,
	unsigned char *nsec,
	const unsigned char *c,
	...
	printf("-- FINALIZATION PHASE --\n");
#endif
	fault_on_finalization_phase( state, k, diff, pos );
	...
	*mlen = clen - CRYPTO_ABYTES;
	...
	printf("faulty tag in the decryption::\n");
	for (i = 0; i < CRYPTO_ABYTES; ++i)
		printf("%02X ", tag[i]);
	printf("\n");
