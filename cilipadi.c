...
unsigned char Rstate[ 8 ], Rstate1[ 8 ], st[ 4 ][ 4 ];
...
int fault_on_permutation_256(unsigned char *state, int rounds, unsigned char diff) {
	...
	for (i = 0; i < rounds; ++i) {
		...
		if( i == rounds-1 ) {
			...
			memcpy(temp, x1, 8);
			fault_on_f_function(temp, 1, i, diff);
			xor_bytes(x2, temp, 8);
		}
		else {
...
int fault_on_f_function(unsigned char *x, int l, int pround, unsigned char diff) {
	...
	for (i = 0; i < 16; ++i) {
		if(i%2) led_state[i/4][i%4] = (x[i>>1]>>4)&0xF;
		else led_state[i/4][i%4] = x[i>>1]&0xF;
	}
...
void copy_temp( unsigned char temp[ 8 ] ) {
...
void copy_x2( unsigned char x2[ 8 ] ) {
...
void copy_led_state( unsigned char led[ 4 ][ 4 ] ) {
...
void printstate256(unsigned char x1[8], unsigned char x2[8], unsigned char x3[8], unsigned char x4[8]) {
	...
	printf("whole state of size 32 byte before the last round:\n");
	for( i = 0; i < 8; ++i )
	 	printf("%02x ", x1[ i ]);
...
int permutation_256_final_phase(unsigned char *state, int rounds) {
	...
	for (i = rounds-1; i < rounds; ++i) {
		...
		memcpy(temp, x1, 8);
		f_function_final_phase(temp, 1, i);
...
int fault_on_permutation_256_final_phase(unsigned char *state, int rounds, unsigned char diff, unsigned char pos) {
	...
	for (i = 0; i < rounds-1; ++i) {
		...
		if( i == rounds-1 ) {
			...
			memcpy(temp, x1, 8);
			fault_on_f_function_final_phase(temp, 1, i, diff, pos);
			xor_bytes(x2, temp, 8);
		}
		else {
	...
	//printf("state (output of permutation_a_n)\n");
	//for (short j=8; j<16; j++) printf("%02x  ", state[j]); printf("\n");
...
int fault_on_f_function_final_phase(unsigned char *x, int l, int pround, unsigned char diff, unsigned char pos) {
	...
	for (i = 1; i < rounds; ++i) {
		...
		//printf("(faulty) state after one round led and s-box:\n");
		//printled( led_state );
...
///////////////////////////////////////////////////////////////
		...
		//printf("state after one round led and s-box:\n");
		//printled( led_state );
